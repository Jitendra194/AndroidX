...
class BottomNavigationDrawerCallback : BottomSheetBehavior.BottomSheetCallback() {
    ...
    override fun onSlide(sheet: View, slideOffset: Float) {
        if (halfExpandedSlideOffset == Float.MAX_VALUE) calculateInitialHalfExpandedSlideOffset(sheet)
    ...
    private fun calculateInitialHalfExpandedSlideOffset(sheet: View) {
        ...
        val peekHeightMin = parent.resources.getDimensionPixelSize(R.dimen.design_bottom_sheet_peek_height_min)
        val peek = max(peekHeightMin, parent.height - parent.width * 9 / 16)
        val collapsedOffset = max(parent.height - peek, max(0, parent.height - sheet.height))
